...
class InfLoRA(BaseLearner):
    ...
    def train_function(self, train_loader, test_loader, optimizer, scheduler):
        ...
        # ----- SAU KHI TRAIN XONG, EVALUATE TRÊN TEST LOADER -----
        self._network.eval()
        with torch.no_grad():
            # y_pred: dự đoán global (không biết task)
            # y_pred_with_task: dự đoán khi biết task-id (oracle task)
            y_pred, y_pred_with_task, y_true, y_pred_task, y_true_task = self._eval_cnn(
                test_loader
            )

            ret_global = self._evaluate(y_pred, y_true)
            ret_taskid = self._evaluate(y_pred_with_task, y_true)

        print(f"Task {self._cur_task} - Test Acc (global): {ret_global['top1']:.2f}")
        print(
            f"Task {self._cur_task} - Test Acc (with task id): {ret_taskid['top1']:.2f}"
        )
